...
export type ResourceSuggestionPayload = {
  moduleTitle: string;
  recommendedResources: ResourceRecommendation[];
  studyAdvice: string;
};

export type ModuleResourceSuggestion = {
  _id: string;
  userId: string;
  moduleId: string;
  type: "resources";
  ...
  outputJson: ResourceSuggestionPayload;
  createdAt: string;
};
...
export async function fetchModuleResources(
  token: string,
  moduleId: string
): Promise<ModuleResourceSuggestion | null> {
  const response = await fetch(buildApiUrl(`/api/modules/${moduleId}/ai/resources`), {
    headers: getAuthHeaders(token),
  });
  if (!response.ok) {
    const data = await parseJsonSafe(response);
    throw new Error(data?.message || "Failed to load AI resources.");
  }
  const data = await parseJsonSafe(response);
...
export async function generateModuleResources(
  ...
  moduleId: string,
  language: "en" | "fr"
): Promise<ModuleResourceSuggestion> {
  const response = await fetch(buildApiUrl(`/api/modules/${moduleId}/ai/resources`), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const data = await parseJsonSafe(response);
    throw new Error(data?.message || "Failed to generate AI resources.");
  }
  const data = await parseJsonSafe(response);
